...
//
// AITER wrapper for SLA sparse backward. Dispatches the CK-tile VSA
// split bwd (dkdv-only + dq-only). Builds the transposed K-major LUT
// required by the dkdv-only half; the dq-only half consumes the
// original M-major LUT directly. When the CK kernel returns a negative
// wall time (kernel failure), the wrapper zero-fills dq/dk/dv to keep
// callers safe.
                                ...
                                std::optional<at::Tensor> dv_)
{
    ...
    // The CK bwd pipeline is instantiated at kM0=64, kN0=64. Callers
    // with BLKQ=128 are handled by expanding each SLA Q-block into
    // q_scale=2 CK Q-tiles in the transposed LUT below.
    ...
    // Sort each (bh, k) row ascending. torch::sort uses GPU radix sort.
    kq_lut = std::get<0>(torch::sort(kq_lut, /*dim=*/-1));

    // The dq-only (Q-major) kernel consumes the M-major LUT directly.
    // Sorting each (b, h, m) row gives monotonic HBM access patterns
    // for K/V fetches — correctness is order-independent, but sorted
    // rows line up with the pipeline's prefetch expectations.
    at::Tensor lut_sorted = std::get<0>(torch::sort(lut, /*dim=*/-1));

    // The split bwd writes dq direct to bf16; no fp32 accumulator scratch.
    ...
    // dq is written directly to bf16/fp16 by the dq-only kernel;
    // dk/dv are written by the dkdv-only kernel's Default2DEpilogue.
    return {dq, dk, dv};
}
