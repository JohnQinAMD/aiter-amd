                                ...
                                std::optional<at::Tensor> out_)
{
    ...
    // LSE output: always allocated (CK codegen now instantiates kStoreLSE=true
    // variants). Shape [B, H, S] fp32, contiguous. Sentinel-fill to detect
    // whether the kernel is writing LSE at all (Stage 7 Tier 1 debug).
    at::Tensor lse = torch::full({B, H, S},
                                  -987654.0f,
