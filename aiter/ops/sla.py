...
Currently exposes only the forward pass via the CK-tile VSA kernel. The SLA
training path still runs through Triton because CK has no matching backward
kernel for block-sparse attention. See
/home/yanyuqin/.claude/plans/spicy-chasing-ritchie.md Stage 7 for the bwd plan.
"""
...
def _gen_sla_fwd_fake_tensors(
    ...
    Returns [out, lse] to mirror the C++ signature (Stage 7 Tier 1 onward).
    """
...
def _cmd_gen_func_sla_bwd(
    ...
    """Module name for JIT codegen. Stage 7 Tier 2 — no variants yet."""
...
def sla_bwd(
    ...
    """SLA sparse-attention backward via CK-tile VSA kernel (Tier 2 scaffolding).

    Returns [dq, dk, dv]. When the underlying CK kernel is not yet ready
    (Tier 2 stub), all three grads are zero-filled so callers can observe
    the fallback without NaNs.
    """
    ...
