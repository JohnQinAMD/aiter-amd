...
def build_kimi_k3_b1_shared_down_fp8_module(
    ...
    weight_is_fp8: bool = True,
    apply_situ: bool = True,
):
    """Build the fused SiTU + BF16-activation shared-down projection."""
    ...
    weight_tag = "fp8" if weight_is_fp8 else "bf16"
    input_tag = "situ" if apply_situ else "activated"
    kernel_name = (
        f"kimi_k3_b1_{input_tag}_shared_down_bf16_{weight_tag}_gfx950"
    ...
    def shared_down_fp8_kernel(
        ...
        activated_lds = fx.SharedAllocator().allocate(SharedStorage).peek().activated.ptr
        ...
        def sigmoid(value):
            ...
            # The native exp2/rcp approximation is faster, but its accumulated
            # shared-down error is outside this kernel's frozen BF16 gate.
        ...
        for activation_iteration in range_constexpr(
            ...
            element = tid + arith.constant(activation_iteration * block_threads, type=i32)
            valid = arith.cmpi(
                CmpIPredicate.ult,
                element,
            ...
            with ir.InsertionPoint(activation_if.then_block):
                if const_expr(apply_situ):
                    ...
                    activated_bf16 = arith.trunc_f(T.bf16, _raw(activated))
                else:
                    activated_bf16 = buffer_ops.buffer_load(
                        gate_up_rsrc,
                        element,
                        vec_width=1,
                        dtype=T.bf16,
        ...
        first_group = ArithValue(gpu.block_idx.x) * arith.constant(waves_per_block, type=i32) + wave
            ...
            for row_offset in range_constexpr(rows_per_wave):
                ...
                with ir.InsertionPoint(row_if.then_block):
                    ...
                    for k_iteration in range_constexpr(
                        ...
                        weight_element = (
                            row * arith.constant(_SHARED_INTERMEDIATE_SIZE, type=i32) + k_element
                        )
                        if const_expr(weight_is_fp8):
                            weight_f32 = load_fp8x4_as_f32(
                                weight_rsrc,
                                weight_element,
                            )
                        else:
                    ...
                    reduced = ArithValue(wave_reduce_add(local_dot)) * ArithValue(row_scale)
