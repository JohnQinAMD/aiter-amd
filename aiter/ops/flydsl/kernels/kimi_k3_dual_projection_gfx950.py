...
def build_kimi_k3_b1_dual_projection_module(
    rows_per_wave: int = 2,
    cu_count: int = 256,
    waves_per_eu: int = 0,
    ...
    """Build a one-launch dual projection with two independent weight pointers."""
    ...
    hidden_load_iterations = (_HIDDEN_SIZE + block_threads * _ELEMENTS_PER_LOAD - 1) // (
    ...
    kernel_name = (
        f"kimi_k3_b1_dual_projection_bf16_gfx950"
                ...
                with ir.InsertionPoint(row_if.then_block):
                    ...
                    for k_iteration in range_constexpr(
                        ...
                        if const_expr(hidden_to_lds):
                            ...
                            hidden_vector = load_bf16x8(hidden_rsrc, k_element)
                        weight_if = scf.IfOp(
                            is_routed,
                            results_=[vec8_bf16],
                            has_else=True,
                        ...
                        with ir.InsertionPoint(weight_if.else_block):
                            weight_element = (
                                shared_row * arith.constant(_HIDDEN_SIZE, type=i32) + k_element
                            )
                            weight_vector = load_bf16x8(
                                shared_weight_rsrc,
                                weight_element,
