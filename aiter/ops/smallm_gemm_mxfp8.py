...
def mxfp8_gemv(
    ...
    """Decode dense MX-FP8 linear (X @ W.T). Returns the [M, N] output, or None
    when the shape is outside the measured-win envelope (caller falls back)."""
    if out_dtype != torch.bfloat16:
        return None  # kernels only emit bf16
    M, K = xq.shape
    N = wq.shape[0]

    # MFMA crossover (M in {8,16,32,64}) is tried first; it falls through on miss.
    if M in _MFMA_M_SET:
