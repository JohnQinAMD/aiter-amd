...
#                        nhead <= 96, batch_size=1. NUM_KV_SPLITS is selected
#                        to fill about 256 WGs unless explicitly overridden.
...
#                        (stage-1 + stage-2 reduce into the final O). Splits are
#                        selected automatically unless explicitly overridden.
#                        A partial last head block masks OOB heads.
#
    ...
    if REGIME == "bh64":
        ...
        if REGIME == "bh16bn128":
            ...
            auto_num_kv_splits = max(
                1,
                min(
                    _MAX_NUM_KV_SPLITS // (batch_size * qlen * NUM_M_BLOCKS),
                    min_kv_seq_len,
                ),
            )
        else:  # bh16bn64
