...
- **`bh16bn128`** (`nhead &le; 96`, `batch_size == 1`, fp8 KV): BLOCK_H=16, BLOCK_N=128, 3-D grid `(1, NUM_KV_SPLITS, head_block * qlen)`. Heads are tiled in blocks of 16. The token-bound automatic policy is `NUM_KV_SPLITS = max(1, min(256 // (qlen * cdiv(nhead, 16)), min_kv_seq_len))`, which fills about 256 WGs while keeping every split non-empty. Optional `kv_scale` dequant. Stage-2 reduce runs whenever `NUM_KV_SPLITS > 1`. Supports the general case `num_iter &isin; {1, 2, ...}` (no `gl.assume(num_iter >= 3)`). A partial last head block masks OOB heads on Q load and O store.
- **`bh16bn64`** (`nhead &le; 96`, bf16 KV): BLOCK_H=16, BLOCK_N=64, 3-D grid `(batch_size, NUM_KV_SPLITS, head_block * qlen)`. The block-bound automatic policy is `NUM_KV_SPLITS = max(1, min(256 // (batch_size * qlen * cdiv(nhead, 16)), cdiv(min_kv_seq_len, BLOCK_N)))`, which fills about 256 WGs but never splits a sequence into more 64-token blocks than it contains. Use when KV is kept in bf16. A partial last head block masks OOB heads. Full decode runs stage 1 and, when `NUM_KV_SPLITS > 1`, the stage-2 reduction into `o`.
...
| nhead | 64 or 128 | &le; 96 | &le; 96 |
| Page size | 1 | 1 | 1 |
| BLOCK_H | 64 | 16 | 16 |
| BLOCK_N | 64 | 128 | 64 |
| MFMA | 16&times;16&times;32, warps=[4,1] | 16&times;16&times;32, warps=[1,4] | 16&times;16&times;32, warps=[1,4] |
| Grid | 3-D XCD-aware | 3-D `(1, NUM_KV_SPLITS, head_block * qlen)` | 3-D `(batch, NUM_KV_SPLITS, head_block * qlen)` |
| NUM_KV_SPLITS | auto &isin; {1, 2, 4} from (batch, nhead, qlen) | `max(1, min(256 // (qlen * cdiv(nhead, 16)), min_kv_seq_len))` | `max(1, min(256 // (batch_size * qlen * cdiv(nhead, 16)), cdiv(min_kv_seq_len, 64)))` |
