...
def _oracle(routed, shared, rms_weight, up_weight):
    inverse_rms = torch.rsqrt(routed.float().square().mean(dim=-1, keepdim=True) + EPSILON)
...
def test_latent_moe_tail_support_predicate_is_narrow():
    ...
    assert not supports_latent_moe_tail(noncontiguous, shared, rms_weight, up_weight, EPSILON)
    assert not supports_latent_moe_tail(routed, shared, rms_weight, up_weight.float(), EPSILON)
    assert not supports_latent_moe_tail(routed, shared, rms_weight, up_weight, float("nan"))
    assert not supports_latent_moe_tail(routed, shared, rms_weight, up_weight, float("inf"))
...
def test_latent_moe_tail_rejects_noncontiguous_input():
    ...
    routed = torch.empty((LATENT_DIM, 2), dtype=torch.bfloat16, device="cuda")[:, 0].unsqueeze(0)
