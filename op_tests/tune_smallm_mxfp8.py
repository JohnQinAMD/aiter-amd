...
def triton_dense(xq, xs, wq, ws):
    """The actual vLLM fallback: M-keyed *tuned* Triton (small M -> 32/64/256
    w4 s2; large M -> 64/128/128 w8). Comparing HIP against this -- not the
    stock tile -- keeps the use_hip decision honest."""
    Mn, K = xq.shape
    N = wq.shape[0]
    out = torch.empty((Mn, N), dtype=torch.bfloat16, device=xq.device)
    if Mn <= 64:
        BM, BN, BK, nw, ns = 32, 64, 256, 4, 2
    else:
        BM, BN, BK, nw, ns = 64, 128, 128, 8, 1
    ...
    _tl[g](xq, xs, wq, ws, out, Mn, N, K, xq.stride(0), xq.stride(1), xs.stride(0),
           ...
           out.stride(0), out.stride(1), BM=BM, BN=BN, BK=BK, num_warps=nw, num_stages=ns)
    return out


# Engage HIP only when it beats the tuned Triton fallback by this margin -- at a
